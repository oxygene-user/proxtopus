...
const BigInt& DL_PublicKey::get_int_field(Any_Algo algo, std::string_view field) const {
   if(field == "p") {
...
const BigInt& DL_PrivateKey::get_int_field(Any_Algo algo, std::string_view field) const {
   if(field == "p") {
      return m_group.get_p();
   } else if(field == "q") {
      return m_group.get_q();
