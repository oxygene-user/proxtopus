...
class BOTAN_PUBLIC_API(2, 0) DER_Encoder final {
   public:
      ...
      */
      ...
      DER_Encoder& raw_bytes(const std::span<const uint8_t>& val) { return raw_bytes(val.data(), val.size()); } ///  PROXTOPUS
      ...
      ///  PROXTOPUS : encode secure_vector
      DER_Encoder& encode(const secure_vector<uint8_t>& vec, ASN1_Type real_type) {
          return encode(vec.data(), vec.size(), real_type);
      }
