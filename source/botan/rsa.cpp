...
bool RSA_PrivateKey::check_key(RandomNumberGenerator& rng, bool strong) const {
   ...
   if(strong) {
      ...
      return KeyPair::signature_consistency_check(rng, *this, Algo_Group(ALG::PSSR, ALG::SHA_256));
#else
      return KeyPair::signature_consistency_check(rng, *this, Algo_Group(ALG::_Raw));
#endif
   }
...
class RSA_Signature_Operation final : public PK_Ops::Signature,
   ...
   public:
      ...
      Hash_Algo hash_function() const override { return m_emsa->hash_function(); }

      RSA_Signature_Operation(const RSA_PrivateKey& rsa, Algo_Group padding, RandomNumberGenerator& rng) :
...
AlgorithmIdentifier RSA_Signature_Operation::algorithm_identifier() const {
   Algo_Group emsa_name = m_emsa->name();

   try {
      Algo_Group full_name = Algo_Group(ALG::RSA) + emsa_name;
      const OID oid = OID::from_ag(full_name);
      return AlgorithmIdentifier(oid, AlgorithmIdentifier::USE_EMPTY_PARAM);
   } catch(Lookup_Error&) {}

   if(emsa_name == ALG::PSSR || emsa_name == ALG::EMSA4) {
      auto parameters = PSS_Params::from_emsa_name(emsa_name).serialize();
      return AlgorithmIdentifier(Algo_Group(ALG::RSA, ALG::PSSR), parameters);
   }
...
class RSA_Decryption_Operation final : public PK_Ops::Decryption_with_EME,
                                       private RSA_Private_Operation {
   public:
      RSA_Decryption_Operation(const RSA_PrivateKey& rsa, Algo_Group eme, RandomNumberGenerator& rng) :
...
class RSA_KEM_Decryption_Operation final : public PK_Ops::KEM_Decryption_with_KDF,
                                           private RSA_Private_Operation {
   public:
      RSA_KEM_Decryption_Operation(const RSA_PrivateKey& key, Algo_Group kdf, RandomNumberGenerator& rng) :
...
class RSA_Encryption_Operation final : public PK_Ops::Encryption_with_EME,
   ...
   public:
      RSA_Encryption_Operation(const RSA_PublicKey& rsa, Algo_Group eme) :
...
class RSA_Verify_Operation final : public PK_Ops::Verification,
   ...
   public:
      ...
      RSA_Verify_Operation(const RSA_PublicKey& rsa, Algo_Group padding) :
...
class RSA_KEM_Encryption_Operation final : public PK_Ops::KEM_Encryption_with_KDF,
                                           private RSA_Public_Operation {
   public:
      RSA_KEM_Encryption_Operation(const RSA_PublicKey& key, Algo_Group kdf) :
...
/// PROXTOPUS : provider removed
std::unique_ptr<PK_Ops::KEM_Encryption> RSA_PublicKey::create_kem_encryption_op(Algo_Group params) const {
      return std::make_unique<RSA_KEM_Encryption_Operation>(*this, params);
}

std::unique_ptr<PK_Ops::Verification> RSA_PublicKey::create_verification_op(Algo_Group params) const {
      return std::make_unique<RSA_Verify_Operation>(*this, params);
}

namespace {

Algo_Group parse_rsa_signature_algorithm(const AlgorithmIdentifier& alg_id) {
   //const auto sig_info = split_on(alg_id.oid().to_formatted_string(), '/');
    Algo_Group sig_info = (Algo_Group)alg_id.oid();

   if(sig_info.empty() || sig_info.size() == 1 || sig_info.first() != ALG::RSA) {
      throw Decoding_Error("Unknown AlgorithmIdentifier for RSA X.509 signatures");
   }

   Algo_Group padding = sig_info.skip_first();

   if(padding == ALG::EMSA4) {
      ...
      Hash_Algo hash_algo = pss_params.hash_function();
      if(hash_algo != ALG::SHA_1 && hash_algo != ALG::SHA_224 && hash_algo != ALG::SHA_256 && hash_algo != ALG::SHA_384 &&
         hash_algo != ALG::SHA_512) {
         throw Decoding_Error("Unacceptable hash for PSS signatures");
      }

      if(pss_params.mgf_function() != ALG::MGF1) {
      ...
      padding += hash_algo.a;
      padding += ALG::MGF1;
      padding.saltl = (uint8_t)(pss_params.salt_length() & 0xff);
   }
...
/// PROXTOPUS : provider removed
std::unique_ptr<PK_Ops::Verification> RSA_PublicKey::create_x509_verification_op(const AlgorithmIdentifier& alg_id) const {
