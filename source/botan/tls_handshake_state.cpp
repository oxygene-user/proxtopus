...
std::unique_ptr<KDF> Handshake_State::protocol_specific_prf() const {
   ...
   auto prf_algo = ciphersuite().prf_algo();

   if(prf_algo == ALG::MD5 || prf_algo == ALG::SHA_1) {
       return KDF::create_or_throw(Algo_Group(ALG::TLS_12_PRF, ALG::SHA_256));
   }
   return KDF::create_or_throw(Algo_Group(ALG::TLS_12_PRF, prf_algo.a));
}

std::pair<Algo_Group, Signature_Format> Handshake_State::choose_sig_format(const Private_Key& key,
                                                                            Signature_Scheme& chosen_scheme,
                                                                            bool for_client_auth,
                                                                            const Policy& policy) const {
   Any_Algo sig_algo = key.algo_name();
   ...
   Hash_Algo hash = chosen_scheme.hash_function_name();
...
bool supported_algos_include(const std::vector<Signature_Scheme>& schemes, Any_Algo key_type, Hash_Algo hash_type) {
...
std::pair<Algo_Group, Signature_Format> Handshake_State::parse_sig_format(
   const Public_Key& key,
   Signature_Scheme scheme,
   const std::vector<Signature_Scheme>& offered_schemes,
   bool for_client_auth,
   const Policy& policy) const {
   Any_Algo key_type = key.algo_name();
   ...
   */
   ...
   Hash_Algo hash_algo = scheme.hash_function_name();
