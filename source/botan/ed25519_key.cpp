...
class Ed25519_Pure_Verify_Operation final : public PK_Ops::Verification {
   public:
      ...
      Hash_Algo hash_function() const override { return Hash_Algo(ALG::SHA_512); }
...
class Ed25519_Hashed_Verify_Operation final : public PK_Ops::Verification {
   public:
      Ed25519_Hashed_Verify_Operation(const Ed25519_PublicKey& key, Hash_Algo hash, bool rfc8032) :
...
AlgorithmIdentifier Ed25519_Pure_Sign_Operation::algorithm_identifier() const {
   return AlgorithmIdentifier(OID::from_ag(Algo_Group(ALG::Ed25519)), AlgorithmIdentifier::USE_EMPTY_PARAM);
}
...
class Ed25519_Hashed_Sign_Operation final : public PK_Ops::Signature {
   public:
      Ed25519_Hashed_Sign_Operation(const Ed25519_PrivateKey& key, Hash_Algo hash, bool rfc8032) :
...
std::unique_ptr<PK_Ops::Verification> Ed25519_PublicKey::create_verification_op(Algo_Group params) const {

    if(params.empty() || params.first() == ALG::_Pure) {
        return std::make_unique<Ed25519_Pure_Verify_Operation>(*this);
    } else if (params.first() == ALG::_Ed25519ph) {
        return std::make_unique<Ed25519_Hashed_Verify_Operation>(*this, Hash_Algo(ALG::SHA_512), true);
    } else {
        return std::make_unique<Ed25519_Hashed_Verify_Operation>(*this, Hash_Algo(params.first()), false);
    }
}

std::unique_ptr<PK_Ops::Verification> Ed25519_PublicKey::create_x509_verification_op(const AlgorithmIdentifier& alg_id) const {
...
std::unique_ptr<PK_Ops::Signature> Ed25519_PrivateKey::create_signature_op(RandomNumberGenerator& /*rng*/,
    ...
    if(params.empty() || params.first() == ALG::_Pure) {
        ...
        return std::make_unique<Ed25519_Hashed_Sign_Operation>(*this, Hash_Algo(ALG::SHA_512), true);
    } else {
        return std::make_unique<Ed25519_Hashed_Sign_Operation>(*this, Hash_Algo(params.first()), false);
    }
}
