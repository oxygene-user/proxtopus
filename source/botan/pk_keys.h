...
class BOTAN_PUBLIC_API(3, 0) Asymmetric_Key {
   public:
      ...
      */
      virtual Any_Algo algo_name() const = 0;
...
class BOTAN_PUBLIC_API(2, 0) Public_Key : public virtual Asymmetric_Key {
   public:
      ...
      */
       ...
       */
      std::string fingerprint_public(HashFunction* hash /*= "SHA-256" */) const;
      ...
      */
      /// PROXTOPUS : provider removed
      virtual std::unique_ptr<PK_Ops::KEM_Encryption> create_kem_encryption_op(Algo_Group params) const;
      ...
      */
      /// PROXTOPUS : provider removed
      virtual std::unique_ptr<PK_Ops::Verification> create_verification_op(Algo_Group params) const;
...
class BOTAN_PUBLIC_API(2, 0) Private_Key : public virtual Public_Key {
   public:
      ...
      */
       ...
       */
      std::string fingerprint_private(HashFunction* hash) const;
...
std::string BOTAN_PUBLIC_API(2, 4) create_hex_fingerprint(const uint8_t bits[], size_t len, HashFunction *hash);

inline std::string create_hex_fingerprint(std::span<const uint8_t> vec, HashFunction *hash) {
   return create_hex_fingerprint(vec.data(), vec.size(), hash);
}
