...
#include <botan/internal/sha2_64.h> /// PROXTOPUS
#include <botan/internal/aes.h> /// PROXTOPUS
#include <botan/internal/gcm.h> /// PROXTOPUS
...
std::vector<uint8_t> Session::encrypt(const SymmetricKey& key, RandomNumberGenerator& rng) const {
    ...
    auto hash = std::make_unique<SHA_512_256>();
    auto hmac = std::make_unique<HMAC>(std::move(hash));
   ...
   auto aead = std::make_unique<GCM_Encryption>(std::move(bc), 16);
...
Session Session::decrypt(std::span<const uint8_t> in, const SymmetricKey& key) {
   try {
      ...
      auto bc = std::make_unique<AES_256>();
      auto aead = std::make_unique<GCM_Decryption>(std::move(bc), 16);
