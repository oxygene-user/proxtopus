...
class BOTAN_PUBLIC_API(2, 0) Policy {
   public:
      ...
      * order of preference.
      */
      virtual Algo_Group allowed_ciphers() const;

      /**
      * Returns a list of hash algorithms we are willing to use for
      * signatures, in order of preference.
      */
      virtual Algo_Group allowed_signature_hashes() const;

      /**
      * Returns a list of MAC algorithms we are willing to use.
      */
      virtual Algo_Group allowed_macs() const;
      ...
      */
      virtual Algo_Group allowed_key_exchange_methods() const;

      /**
      * Returns a list of signature algorithms we are willing to
      * use, in order of preference.
      */
      virtual Algo_Group allowed_signature_methods() const;
      ...
      */
      ...
      bool allowed_signature_method(Auth_Method sig_method) const;
      bool allowed_signature_hash(Hash_Algo hash) const;
...
class BOTAN_PUBLIC_API(2, 7) NSA_Suite_B_192 : public Policy {
   public:
       Algo_Group allowed_ciphers() const override { return Algo_Group(ALG::AES_256_GCM); }

      Algo_Group allowed_signature_hashes() const override {
         return Algo_Group(ALG::SHA_384);
      }

      Algo_Group allowed_macs() const override { return Algo_Group(Mac_Algo::AEAD); }

      Algo_Group allowed_key_exchange_methods() const override {
         return Algo_Group(ALG::ECDH);
      }

      Algo_Group allowed_signature_methods() const override {
         return Algo_Group(ALG::ECDSA);
      }
...
class BOTAN_PUBLIC_API(2, 0) Strict_Policy : public Policy {
   public:
       Algo_Group allowed_ciphers() const override;

      Algo_Group allowed_signature_hashes() const override;

      Algo_Group allowed_macs() const override;
