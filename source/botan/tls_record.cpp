...
Connection_Cipher_State::Connection_Cipher_State(Protocol_Version version,
   ...
   if(nonce_format() == Nonce_Format::CBC_MODE) {
      ...
      // legacy CBC+HMAC mode
       ...
       auto hash = HashFunction::create_or_throw(suite.mac_algo().a);
       auto mac = std::make_unique<HMAC>(std::move(hash));
...
void write_record_header(secure_vector<uint8_t>& output,
   ...
   output.clear_fast(); /// PROXTOPUS : no need to zeroise bytes here due we reuse buffer
...
void write_unencrypted_record(secure_vector<uint8_t>& output,
   ...
   output += std::span<const uint8_t>(message, message_len); /// PROXTOPUS
}
...
size_t fill_buffer_to(
   ...
   readbuf += std::span<const uint8_t>(input, taken); /// PROXTOPUS
