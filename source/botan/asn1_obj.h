...
class BOTAN_PUBLIC_API(2, 0) OID final : public ASN1_Object {
   public:
      ...
      */
      ...
      explicit OID(oid_index i) :m_index(i) {}
      oid_index index() const { return m_index; }
#if 0
      /**
      ...
      */
      ...
      static OID from_ag(Algo_Group ag);
      Algo_Group alg() const
      {
          return oid_core(m_index).alg;
      }
      ...
      */
      bool empty() const { return m_index == oid_index::_empty; }
      ...
      */
      ...
      operator Algo_Group() const
      {
          return g_oids[static_cast<size_t>(m_index)].alg;
      }
      ...
      */
      bool operator==(const OID& other) const { return m_index == other.m_index; }
      bool operator<(const OID& other) const { return m_index < other.m_index; }
      ...
      */
      ...
      bool operator==(oid_index i) const { return m_index == i; }
...
inline std::optional<PrimeOrderCurveId> PrimeOrderCurveId::from_oid(const OID& oid)
{
    if (oid.alg().empty())
        return {};
    return PrimeOrderCurveId(oid.alg().first().a);
...
class BOTAN_PUBLIC_API(2, 0) AlgorithmIdentifier final : public ASN1_Object {
   public:
      ...
      AlgorithmIdentifier(Algo_Group oid_name, Encoding_Option enc);

      AlgorithmIdentifier(const OID& oid, const std::vector<uint8_t>& params);
      AlgorithmIdentifier(Algo_Group oid_name, const std::vector<uint8_t>& params);
