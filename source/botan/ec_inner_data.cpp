...
EC_Group_Data::EC_Group_Data(const BigInt& p,
   ...
   if(!m_oid.empty()) {
      ...
      m_pcurve = PCurve::PrimeOrderCurve::for_named_curve(oid.alg().first().a);
      if(m_pcurve) {
...
std::function<void(std::span<uint8_t>)> h2c_expand_message(Hash_Algo hash_fn,
...
std::unique_ptr<EC_AffinePoint_Data> EC_Group_Data::point_hash_to_curve_ro(Hash_Algo hash_fn,
                                                                           std::span<const uint8_t> input,
...
std::unique_ptr<EC_AffinePoint_Data> EC_Group_Data::point_hash_to_curve_nu(Hash_Algo hash_fn,
                                                                           std::span<const uint8_t> input,
                                                                           std::span<const uint8_t> domain_sep) const {
   if(m_pcurve) {
