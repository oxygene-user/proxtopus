...
Algo_Group Policy::allowed_ciphers() const {

    return Algo_Group( ALG::CHACHA20_POLY1305, ALG::AES_256_GCM, ALG::AES_128_GCM );
}

Algo_Group Policy::allowed_signature_hashes() const {
    return Algo_Group(Hash_Algo::SHA_512, Hash_Algo::SHA_384, Hash_Algo::SHA_256);
}

Algo_Group Policy::allowed_macs() const {

    /// PROXTOPUS : avoid use of strings
    return Algo_Group( Mac_Algo::AEAD, Mac_Algo::SHA_256, Mac_Algo::SHA_384, Mac_Algo::SHA_1 );
...
Algo_Group Policy::allowed_key_exchange_methods() const {

    return Algo_Group(Kex_Algo::ECDH, Kex_Algo::DH);
/*
   return {
      //"ECDHE_PSK",
      //"PSK",
...
Algo_Group Policy::allowed_signature_methods() const {
    return Algo_Group(Auth_Method::ECDSA, Auth_Method::RSA);
        /*
   return {
      "ECDSA", "RSA",
      //"IMPLICIT",
...
bool Policy::allowed_signature_method(Auth_Method sig_method) const {
   return value_exists(allowed_signature_methods(), sig_method);
}

bool Policy::allowed_signature_hash(Hash_Algo sig_hash) const {
...
void Policy::check_peer_key_acceptable(const Public_Key& public_key) const {
   Any_Algo algo_name = public_key.algo_name();

   const size_t keylength = public_key.key_length();
   size_t expected_keylength = 0;

   if(algo_name == Any_Algo::RSA) {
      expected_keylength = minimum_rsa_bits();
   } else if(algo_name == Any_Algo::DH) {
      expected_keylength = minimum_dh_group_size();
   } else if(algo_name == Any_Algo::ECDH || algo_name == Any_Algo::X25519 || algo_name == Any_Algo::X448) {
      expected_keylength = minimum_ecdh_group_size();
   } else if(algo_name == Any_Algo::ECDSA) {
   ...
   if(keylength < expected_keylength) {
      throw TLS_Exception(Alert::InsufficientSecurity,
                          "Peer sent " + std::to_string(keylength) + " bit " + algo_name.to_string() +
                             " key"
...
class Ciphersuite_Preference_Ordering final {
   public:
      Ciphersuite_Preference_Ordering(const Algo_Group ciphers,
          Algo_Group macs,
          Algo_Group kex,
          Algo_Group sigs) :
...
std::vector<uint16_t> Policy::ciphersuite_list(Protocol_Version version) const {
   const Algo_Group ciphers = allowed_ciphers();
   const Algo_Group macs = allowed_macs();
   const Algo_Group kex = allowed_key_exchange_methods();
   const Algo_Group sigs = allowed_signature_methods();
...
Algo_Group Strict_Policy::allowed_ciphers() const {
    return Algo_Group( ALG::CHACHA20_POLY1305, ALG::AES_256_GCM, ALG::AES_128_GCM );
}

Algo_Group Strict_Policy::allowed_signature_hashes() const {
   return Algo_Group(Hash_Algo::SHA_512, Hash_Algo::SHA_384);
}

Algo_Group Strict_Policy::allowed_macs() const {
   return Algo_Group(Mac_Algo::AEAD);
}

Algo_Group Strict_Policy::allowed_key_exchange_methods() const {
   return Algo_Group(Kex_Algo::ECDH);
}
