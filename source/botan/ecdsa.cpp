...
bool ECDSA_PrivateKey::check_key(RandomNumberGenerator& rng, bool strong) const {
   ...
   return KeyPair::signature_consistency_check(rng, *this, Algo_Group(ALG::SHA_256));
}
...
class ECDSA_Signature_Operation final : public PK_Ops::Signature_with_Hash {
   public:
      ECDSA_Signature_Operation(const ECDSA_PrivateKey& ecdsa, Algo_Group padding, RandomNumberGenerator& rng) :
...
AlgorithmIdentifier ECDSA_Signature_Operation::algorithm_identifier() const {
   auto full_name = Algo_Group(ALG::ECDSA) + hash_function();
...
class ECDSA_Verification_Operation final : public PK_Ops::Verification_with_Hash {
   public:
      ECDSA_Verification_Operation(const ECDSA_PublicKey& ecdsa, Algo_Group padding) :
      ...
      ECDSA_Verification_Operation(const ECDSA_PublicKey& ecdsa, const AlgorithmIdentifier& alg_id) :
            PK_Ops::Verification_with_Hash(alg_id, Auth_Method(ALG::ECDSA), true),
            m_group(ecdsa.domain()),
...
std::unique_ptr<PK_Ops::Verification> ECDSA_PublicKey::create_verification_op(Algo_Group params) const {
