...
std::vector<X509_Certificate> Credentials_Manager::cert_chain(Algo_Group /*unused*/,
...
std::vector<X509_Certificate> Credentials_Manager::cert_chain_single_type(
    Auth_Method cert_key_type,
   const std::vector<AlgorithmIdentifier>& cert_signature_schemes,
   const std::string& type,
   const std::string& context) {
   return find_cert_chain(Algo_Group(cert_key_type), cert_signature_schemes, std::vector<X509_DN>(), type, context);
}
