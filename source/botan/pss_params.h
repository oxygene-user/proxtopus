...
class BOTAN_PUBLIC_API(3, 7) PSS_Params final : public ASN1_Object {
   public:
      ...
      * minor release.
      */
      static PSS_Params from_emsa_name(Algo_Group emsa_name);

      PSS_Params(Hash_Algo hash_fn, size_t salt_len);
      ...
      */
      ...
      Any_Algo hash_function() const { return ((Algo_Group)hash_algid().oid()).first(); }

      Any_Algo mgf_function() const { return ((Algo_Group)mgf_algid().oid()).first(); }
