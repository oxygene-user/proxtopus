...
std::unique_ptr<Public_Key> load_public_key(const AlgorithmIdentifier& alg_id,
   ...
   Algo_Group alg = (Algo_Group)alg_id.oid();
...
#if 0
std::unique_ptr<Private_Key> create_private_key(Any_Algo alg_name,
                                                RandomNumberGenerator& rng,
                                                uint32_t params) {
   /*
   ...
   */
    ...
    case ALG::RSA:
        {
        const size_t modulus_bits = params==0 ? 3072 : params;
...
#endif
    {
        Algo_Group group_id = [&]() -> Algo_Group {
            if (params != 0) {
                //return std::string(params);
                __debugbreak(); // TODO : encode bits size into ALG
            }
            if (alg_name.a == ALG::DSA)
                return Algo_Group(ALG::dsa_botan_2048);

            return Algo_Group(ALG::modp_ietf_2048);
        }();

        DL_Group modp_group(group_id);

#if defined(BOTAN_HAS_DIFFIE_HELLMAN)
        if (alg_name.a == ALG::DH) {
   ...
   BOTAN_UNUSED(alg_name, rng, params);
