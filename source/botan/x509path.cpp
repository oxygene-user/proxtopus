      ...
      if(!subject.signature_algorithm().oid().registered_oid()) {
         ...
         if(issuer_key) {
            ...
            if(sig_status.first != Certificate_Status_Code::VERIFIED) {
               ...
               Hash_Algo hash_used_for_signature = sig_status.second;
...
Certificate_Status_Code PKIX::build_certificate_path(std::vector<X509_Certificate>& cert_path,
   ...
   */
   ...
   certs_seen.insert(end_entity.fingerprint(Hash_Algo::SHA_256));
   ...
   for(;;) {
      ...
      const std::string fprint = issuer->fingerprint(Hash_Algo::SHA_256);
...
Certificate_Status_Code PKIX::build_all_certificate_paths(std::vector<std::vector<X509_Certificate>>& cert_paths_out,
   ...
   */
   ...
   while(!stack.empty()) {
      ...
      if(last == std::nullopt) {
         stack.pop_back();
         std::string fprint = path_so_far.back().fingerprint(Hash_Algo::SHA_256);
      ...
      // process next cert on the path
         ...
         const std::string fprint = last->fingerprint(Hash_Algo::SHA_256);
...
Path_Validation_Restrictions::Path_Validation_Restrictions(bool require_rev,
   ...
   if(key_strength <= 80) {
      m_trusted_hashes.insert(Hash_Algo::SHA_1);
   }

   m_trusted_hashes.insert(Hash_Algo::SHA_224);
   m_trusted_hashes.insert(Hash_Algo::SHA_256);
   m_trusted_hashes.insert(Hash_Algo::SHA_384);
   m_trusted_hashes.insert(Hash_Algo::SHA_512);
