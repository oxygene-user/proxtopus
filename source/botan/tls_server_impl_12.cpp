...
uint16_t choose_ciphersuite(const Policy& policy,
                            ...
                            const std::map<Auth_Method, std::vector<X509_Certificate>>& cert_chains,
                            const Client_Hello_12& client_hello) {
   ...
   */
   ...
   for(auto suite_id : pref_list) {
      ...
      if(suite->signature_used()) {
         Auth_Method sig_algo = suite->sig_algo();
...
std::map<Auth_Method, std::vector<X509_Certificate>> get_server_certs(
   std::string_view hostname, const std::vector<Signature_Scheme>& cert_sig_schemes, Credentials_Manager& creds) {
   Auth_Method cert_types[] = {Auth_Method(ALG::RSA), Auth_Method(ALG::ECDSA), Auth_Method(ALG::DSA)};

   std::map<Auth_Method, std::vector<X509_Certificate>> cert_chains;

   for(size_t i = 0; i < std::size(cert_types); ++i) {
...
void Server_Impl_12::session_create(Server_Handshake_State& pending_state) {
   ...
   if(pending_suite.signature_used() || pending_suite.kex_method() == Kex_Algo::STATIC_RSA) {
      Auth_Method algo_used = pending_suite.signature_used() ? pending_suite.sig_algo() : Auth_Method(ALG::RSA);
