...
#include <botan/internal/sha1.h> /// PROXTOPUS
...
std::unique_ptr<X509_Certificate_Data> parse_x509_cert_body(const X509_Object& obj) {
      ...
      if(ext->get_is_ca() == true) {
         ...
         */
         const bool allowed_by_ext_ku = [](const std::vector<OID>& ext_ku) -> bool {
            ...
            const auto server_auth = OID(oid_index::_1_3_6_1_5_5_7_3_1); // OID::from_name("PKIX.ServerAuth");
            const auto client_auth = OID(oid_index::_1_3_6_1_5_5_7_3_2); // OID::from_name("PKIX.ClientAuth");
            const auto ocsp_sign = OID(oid_index::_1_3_6_1_5_5_7_3_9); // OID::from_name("PKIX.OCSPSigning");
   ...
   // name checking.
   const auto san_oid = OID(oid_index::_2_5_29_17); // OID::from_string("X509v3.SubjectAlternativeName");
   ...
   if(auto sha1 = std::make_unique<SHA_1>()/*HashFunction::create("SHA-1")*/) {
      ...
      data->m_fingerprint_sha1 = create_hex_fingerprint(full_encoding, sha1.get());
   }

   if(auto sha256 = std::make_unique<SHA_256>()/*HashFunction::create("SHA-256")*/) {
      ...
      data->m_fingerprint_sha256 = create_hex_fingerprint(full_encoding, sha256.get());
   }
   ...
   switch(usage) {
      ...
      case Usage_Type::TLS_SERVER_AUTH:
         return (allowed_usage(Key_Constraints::KeyAgreement) || allowed_usage(Key_Constraints::KeyEncipherment) ||
                ...
                allowed_extended_usage(OID(oid_index::_1_3_6_1_5_5_7_3_1) /*"PKIX.ServerAuth"*/);
      ...
      case Usage_Type::TLS_CLIENT_AUTH:
         return (allowed_usage(Key_Constraints::DigitalSignature) || allowed_usage(Key_Constraints::KeyAgreement)) &&
                allowed_extended_usage(OID(oid_index::_1_3_6_1_5_5_7_3_2) /*"PKIX.ClientAuth"*/);
      ...
      case Usage_Type::OCSP_RESPONDER:
         return (allowed_usage(Key_Constraints::DigitalSignature) || allowed_usage(Key_Constraints::NonRepudiation)) &&
                has_ex_constraint(OID(oid_index::_1_3_6_1_5_5_7_3_9) /*"PKIX.OCSPSigning"*/);
...
*/
bool X509_Certificate::is_critical(const OID &ex_name) const {
   return v3_extensions().critical_extension_set(ex_name);
}
...
std::vector<std::string> get_cert_user_info(std::string_view req, const X509_DN& dn, const AlternativeName& alt_name) {
   ...
   auto f = X509_DN::deref_info_field(req);

   if(dn.has_field(f)) {
      return dn.get_attribute(f);
...
std::string X509_Certificate::fingerprint(Hash_Algo hasht) const {
   /*
   ...
   */
   if(hasht == Hash_Algo::SHA_256 && !data().m_fingerprint_sha256.empty()) {
      return data().m_fingerprint_sha256;
   } else if(hasht == Hash_Algo::SHA_1 && !data().m_fingerprint_sha1.empty()) {
      return data().m_fingerprint_sha1;
   } else {
        auto h = HashFunction::create_or_throw(hasht);
        return create_hex_fingerprint(this->BER_encode(), h.get());
   }
}
