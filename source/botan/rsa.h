...
class BOTAN_PUBLIC_API(2, 0) RSA_PublicKey : public virtual Public_Key {
   public:
      ...
      */
      ...
      // sign
      Any_Algo algo_name() const override { return Any_Algo(ALG::RSA); }
      ...
      */
      ...
      std::unique_ptr<PK_Ops::Encryption> create_encryption_op(RandomNumberGenerator& rng,
          Algo_Group params) const override;

      /// PROXTOPUS : provider removed
      std::unique_ptr<PK_Ops::KEM_Encryption> create_kem_encryption_op(Algo_Group params) const override;

      /// PROXTOPUS : provider removed
      std::unique_ptr<PK_Ops::Verification> create_verification_op(Algo_Group params) const override;

      /// PROXTOPUS : provider removed
      std::unique_ptr<PK_Ops::Verification> create_x509_verification_op(const AlgorithmIdentifier& alg_id) const override;
