...
*/
template <typename BYTEBUF, typename T>
void append_tls_length_value(BYTEBUF& buf, const T* vals, size_t vals_size, size_t tag_size) {
...
template <typename Alloc, typename ADB>
void append_tls_length_value(std::vector<uint8_t, Alloc>& buf, const ADB& vals, size_t tag_size) {
   append_tls_length_value(buf, vals.data(), vals.size(), tag_size);
}

template <typename ADB>
void append_tls_length_value(secure_vector<uint8_t>& buf, const ADB& vals, size_t tag_size) {
    append_tls_length_value(buf, vals.data(), vals.size(), tag_size);
}
