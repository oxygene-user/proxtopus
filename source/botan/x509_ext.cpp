...
*/
Subject_Key_ID::Subject_Key_ID(const std::vector<uint8_t>& pub_key, Hash_Algo hasht) {
   auto hash = HashFunction::create_or_throw(hasht);
...
void Name_Constraints::validate(const X509_Certificate& subject,
   ...
   if(!m_name_constraints.permitted().empty() || !m_name_constraints.excluded().empty()) {
      ...
      const bool issuer_name_constraint_critical = subject.is_critical(OID(oid_index::_2_5_29_30) /*"X509v3.NameConstraints"*/);
...
std::vector<uint8_t> Authority_Information_Access::encode_inner() const {
   ...
   if(!m_ocsp_responder.empty()) {
      ...
      der.start_sequence()
         .encode(OID(oid_index::_1_3_6_1_5_5_7_48_1) /*"PKIX.OCSP"*/)
   ...
   for(const auto& ca_isser : m_ca_issuers) {
      ...
      der.start_sequence()
         .encode(OID(oid_index::_1_3_6_1_5_5_7_48_2) /*"PKIX.CertificateAuthorityIssuers"*/)
...
void Authority_Information_Access::decode_inner(const std::vector<uint8_t>& in) {
   ...
   while(ber.more_items()) {
      ...
      if(oid == oid_index::_1_3_6_1_5_5_7_48_1 /*"PKIX.OCSP"*/) {
      ...
      if(oid == oid_index::_1_3_6_1_5_5_7_48_2 /*"PKIX.CertificateAuthorityIssuers"*/) {
         BER_Object name = info.get_next_object();
