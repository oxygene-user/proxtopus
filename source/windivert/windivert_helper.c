    ...
    PWINDIVERT_FILTER object)
{
    ...
    object->field = WinDivertKindToField((KIND)var->kind);
    ...
    char *object, UINT obj_len, const char **error, UINT *error_pos)
{
    ...
    {
        WINDIVERT_FILTER *filter_obj = (WINDIVERT_FILTER*)HeapAlloc(pool, 0,
    ...
    UINT packet_len, const WINDIVERT_ADDRESS *addr)
{
    ...
    err = WinDivertCompileFilter(filter, pool, (WINDIVERT_LAYER)addr->Layer, object, &obj_len);
    if (IS_ERROR(err))
    {
        SetLastError(ERROR_INVALID_PARAMETER);
        goto WinDivertHelperEvalFilterError;
    ...
    result = WinDivertExecuteFilter(
        object,
        (WINDIVERT_LAYER)addr->Layer,
        addr->Timestamp,
        (WINDIVERT_EVENT)addr->Event,
