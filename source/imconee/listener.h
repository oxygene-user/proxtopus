...
public:
	listener(loader& ldr, const std::string& name, const asts& bb);
	/*virtual*/ ~listener() {}

	virtual void open() = 0;
	virtual void stop() = 0;

	static listener* build(loader& ldr, const std::string& name, const asts& bb);
};
...
public:
	...
	tcp_listener(loader& ldr, const std::string& name, const asts& bb);
