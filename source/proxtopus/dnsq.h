...
class dns_resolver
{
    ...
    struct cachemap : public tools::shashmap<char, ptr::shared_ptr<cache_rec>>
    {
        ...
        {
            auto [it, inserted] = try_emplace(h, p);
            if (inserted)
                return;

            if (it->second->add_ip(*p))
                p = it->second; // replace cache_rec in p to old one
        }
        void set(str::astr_view h, ptr::shared_ptr<cache_rec> &p)
        {
            auto [it, inserted] = insert(std::pair(h, p));
            if (inserted)
                return;
    ...
    struct nameserver : ptr::sync_shared_object
    {
        ...
        nameserver(const str::shared_str::ptr& name, const netkit::ipap& ip_) :name(name), ip(ip_) { ASSERT(ip_.has_port()); }
        nameserver(const str::shared_str::ptr& name) :name(name) {}
        bool ip_not_set() const { return !ip.has_port(); }
    };
