...
#ifdef DETAILED_LOG
#define DL(...) LOG_D(__VA_ARGS__) DT
#define LL1(...) LOG_D(__VA_ARGS__) DT
#else
#define DL(...)
#ifdef LOG_LEVEL_1
#define LL1 LOG_D
...
dnspp::parse_result dnspp::parser::start()
{
	...
	if (netkit::to_he(hdr->id) != ((spinlock::tid_self() >> 4) & 0xffff))
		return parse_data_error;
	if (!hdr->qr)
		return parse_data_error;
	//if (hdr->tc)
	...
	signed_t nques = netkit::to_he(hdr->nques);
	psize[0] = netkit::to_he(hdr->nansw);
	psize[1] = netkit::to_he(hdr->nauth);
	psize[2] = netkit::to_he(hdr->nainf);
...
bool dns_resolver::find_ns(query_internals& qi, signed_t deep)
{
	...
	for (str::token<char, str::sep_onechar_rev<char, '.'>> t(qi.cur_host()); t; t())
	{
		bool swlv = false;
		...
		while (curz)
		{
			...
			if (curz != nullptr && curz->servers.size() != 0)
			{
				...
				for (signed_t cnt = curz->servers.size();;)
				{
					...
					if (usable(ns.get()))
					{
						bool samehostandresolver = ns->name->equals(qi.cur_host());
						...
						if (ns->ip_not_set())
						{
							...
							DL("select resolver for %s : %s (no ip)", qi.cur_host().c_str(), ns->name->to_string().c_str());
							...
							for (signed_t tcount = 3; tcount >= 0; --tcount)
							{
								...
								else
								{
									...
									DL("can't resolve resolver's ip (%s)", ns->name->to_string().c_str());
						...
						DL("select resolver for %s : %s (%s)", qi.cur_host().c_str(), ns->name->to_string().c_str(), ns->ip.to_string(true).c_str());
...
void dns_resolver::add_zone_ns(time_t ct, zones_array* zar, newns& ns, const str::astr_view& zone_dom, zone* parent, const proxy* prx)
{
	...
	for (const auto& z : *zar)
	{
		if (z->z == *t)
		{
			...
			for (auto& ip : ns.ips)
			{
				...
				z->servers.emplace_back(nns);
				DL("add zone ns %s (\"%s\") with ip: %s", ns.name->to_string().c_str(), ns.zone.c_str(), ip.to_string(false).c_str());
			}
			if (ns.ips.empty() && !found)
			{
				...
				nns->decaytime = ct + ns.ttl;
				z->servers.emplace_back(nns);
				DL("add zone ns (\"%s\") unresolved for now: %s", ns.zone.c_str(), ns.name->to_string().c_str());
			}
	...
	for (auto& ip : ns.ips)
	{
		...
		DL("new zone ns %s (\"%s\") with ip: %s", ns.name->to_string().c_str(), ns.zone.c_str(), ip.to_string(false).c_str());
	}
	if (ns.ips.empty())
	{
		nameserver* nns = new nameserver(nm);
		...
		nns->decaytime = ct + ns.ttl;
		a->servers.emplace_back(nns);
		DL("new zone ns (\"%s\") unresolved for now: %s", ns.zone.c_str(), ns.name->to_string().c_str());
	}
...
netkit::ipap dns_resolver::resolve(const str::astr& hn_, bool log_it)
{
	...
	case query_internals::r_label2long:
		if (log_it)
			LOG_E("dns: name not legal (label too long): %s", qi.cur_host().c_str());
		break;
	case query_internals::r_request2big:
		if (log_it)
			LOG_E("dns: request too big: %s", qi.cur_host().c_str());
		break;
	case query_internals::r_2manycnames:
		if (log_it)
			LOG_E("dns: resolve failed for domain %s (too many cnames)", qi.hns[0].c_str());
		break;
	case query_internals::r_notresolved:
		if (log_it)
			LOG_E("dns: name not found [%s]", qi.cur_host().c_str());
		break;
	case query_internals::r_networkfail:
		if (log_it)
			LOG_E("dns: all request attempts have been exhausted (network problems?) [%s]", qi.cur_host().c_str());
		break;
	}

	LL1("query failed \"%s\"", qi.hns[0].c_str());
...
ptr::shared_ptr<dns_resolver::cache_rec> dns_resolver::resolve(query_internals &qi, bool lock_resolving)
{
	...
	auto cr = cache.lock_read();
	const auto ce = cr().find(qi.cur_host());
	...
	if (lock_resolving)
	{
		LL1("query \"%s\"", qi.cur_host().c_str());

		if (auto p = start_resolving(qi.cur_host()))
			return p;
	}
	...
	{
		auto r = empty_result(qi.hns.data(), qi.hn_count);
		done_resolve(qi.hns.data(), qi.hn_count);
		qi.result = ri;
		return r;
	};

	dnspp dns;
	if (!dns.prepare(qi.cur_host()))
	{
	...
	next_try:
		...
		DL("query: %s ? %s (%s)", qi.cur_host().c_str(), qi.last_ns->name->to_string().c_str(), qi.ns[qi.used_ips - 1]->ip.to_string(true).c_str());
		...
		for (signed_t tcount = 3; tcount >= 0; --tcount)
		{
			...
			DL("query failed: %s ? %s (%s)", qi.cur_host().c_str(), qi.last_ns->name->to_string().c_str(), qi.ns[qi.used_ips - 1]->ip.to_string(true).c_str());
		...
		for (pr = parser.start(); pr == dnspp::parse_ok; pr = parser.next())
		{
			...
			case dnspp::qtype_cname:
				...
				if (parser.host == qi.cur_host() || parser.host == cname)
				{
					rec_prepare();
					cname = parser.cname;

					DL("cname: %s -> %s", qi.cur_host().c_str(), parser.cname.c_str());

					auto cw = cache.lock_write();
					cw().set(qi.cur_host(), rec);
			...
			case dnspp::qtype_a:
				...
				if (parser.cname.length() == 4)
				{
					...
					if (!ns)
					{
						...
						DL("resolved %s with ip %s", parser.host.c_str(), ip.to_string(false).c_str());
		...
		if (pr == dnspp::parse_done)
		{
			...
			if (rec)
			{
				if (rec->is_empty())
				{
					// cname
					ASSERT(!cname.empty());
					if (qi.hn_count == dnspp::maximum_cnames)
					{
						return not_resolved(query_internals::r_2manycnames);
					}
					qi.hns[qi.hn_count++] = cname;
					dns.prepare(cname);
					rec = nullptr;
				}
				else
				{
					LL1("resolved %s -> %s", qi.cur_host().c_str(), rec->to_string(false).c_str());
					done_resolve(qi.hns.data(), qi.hn_count);
					return rec;
				}
