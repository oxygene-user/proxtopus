...
struct block_header_s
{
    ...
    static block_header_s* get_self(void* p);
    char* usable_space();
    char* pre_block();
    char* post_block(size_t datasize);

    void fill_new(size_t datasize);
    void fill_post(size_t datasize);
...
enum
{
    preblock_size_unaligned = MEMSPY_CORRUPT_CHECK_ZONE_BEGIN + sizeof(block_header_s),
    preblock_size = ((preblock_size_unaligned + 15) & (~15)) - sizeof(block_header_s),

    postblock_size = MEMSPY_CORRUPT_CHECK_ZONE_END,
};

inline block_header_s* block_header_s::get_self(void* p)
{
    return (block_header_s*)((char*)p - sizeof(block_header_s) - preblock_size);
}

inline char* block_header_s::usable_space()
{
    return ((char*)(this + 1)) + preblock_size;
}

inline char* block_header_s::pre_block()
{
    if constexpr (preblock_size == 0)
        return nullptr;
    else
    {
        return (char *)(this + 1);
...
inline char* block_header_s::post_block(size_t datasize)
{
    if constexpr (postblock_size == 0)
        return nullptr;
    else
    {
        return usable_space() + datasize;
    }
}

inline void block_header_s::fill_post(size_t datasize)
{
    if (void* p = post_block(datasize))
        memset(p, 0xEE, postblock_size);
}

inline void block_header_s::fill_new(size_t datasize)
{
    if (void* p = pre_block())
...
void block_header_s::check_corrupt()
{
    if (unsigned char* p = (unsigned char*)pre_block())
        for (int i = 0; i < preblock_size; ++i)
            if (p[i] != 0xBB)
                __debugbreak();

    if (unsigned char* p = (unsigned char*)post_block(size))
        for (int i = 0; i < postblock_size; ++i)
            if (p[i] != 0xEE)
                __debugbreak();
}
...
{
    size_t real_alloc_size = sz + sizeof(block_header_s) + preblock_size + postblock_size;
    char *p = (char *)MEMSPY_SYS_ALLOC(real_alloc_size);
    block_header_s *self = (block_header_s *)p;
    self->fill_new(sz);
...
void block_header_s::mf(void *p)
{
    ...
    block_header_s* me = block_header_s::get_self(p);
    void *real_p = ((char *)me);
    spylock();
    ...
    while( freesize > MEMSPY_MAX_FREE_UNALLOCATED && first_free )
    {
        ...
        void *real_x = (char *)x;
#ifndef _DEBUG
        size_t my_size = x->size + sizeof(block_header_s) + preblock_size + postblock_size;
...
#ifndef _DEBUG
        size_t my_size = me->size + sizeof(block_header_s) + preblock_size + postblock_size;
...
struct memleak_fin_s
{
    ~memleak_fin_s()
    {
        ...
        SymInitialize(block_header_s::process, NULL, TRUE);
#endif
        MEMSPY_DEBUG_OUT("   -================[MEMORY LEAKS]================-\r\n");

        for (block_header_s *b = first; b; b = b->next)
        {
            if (b->getinfo(buf, 2048) < 0) break;
            MEMSPY_DEBUG_OUT(buf);
        }
